...
class FileObjectImageInfo(pytsk3.Img_Info):
    ...
    Thread-safety: pytsk3 may invoke read() concurrently from multiple threads, which
    is true under free-threaded Python; possible even with the GIL via cooperative
    thread switches across the seek+read pair. Most file-like objects implement read
    positioning as a stateful seek+read on a single file descriptro, so concurrent
    calls would race. We serialize the seek/read pair under a per-instance lock so that
    pytsk3 callers can share one FileObjectImageInfo across threads.
    """

    def __init__(self, file_object, file_size, image_type=pytsk3.TSK_IMG_TYPE_RAW):
        ...
        # Guards the seek+read pair on _file_object. Created before the parent __init__
        # because pytsk3 may begin proxying read() calls as soon as the constructor
        # returns from Python's perspective.
        self._read_lock = threading.Lock()

        # Using the old parent class invocation style otherwise some versions of pylint
        # complain also setting type to RAW to make sure Img_Info does not do detection.
